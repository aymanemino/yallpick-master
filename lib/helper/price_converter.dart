...
class PriceConverter {
  ...
  static String percentageCalculation(BuildContext context, double price, double discount, String discountType) {
    // Checking if the discount is a percentage or a flat amount
    if (discountType == 'percent' || discountType == 'percentage') {
      return '${discount.toStringAsFixed(0)}% OFF';  // No decimal places, just the percentage
    } else {
      String currencySymbol = Provider.of<SplashProvider>(context, listen: false).myCurrency.symbol;  // Getting the currency symbol
      return '$currencySymbol ${discount.toStringAsFixed(0)} OFF';  // No decimal places for flat discount, showing the currency symbol
    }
  }
