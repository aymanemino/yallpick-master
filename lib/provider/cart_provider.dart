...
class CartProvider extends ChangeNotifier {
  ...
  void getShippingMethodWithoutChosen(
      BuildContext context, List<List<CartModel>> cartProdList) async {
    print('-------$_getData');
    _isLoading = true;
    _getData = false;
    ...
    for (int i = 0; i < sellerIdList.length; i++) {
      ...
      if (apiResponse.response != null &&
        ...
        _shippingList[i].shippingMethodList = [];
        _shippingList[i].shippingMethodList.addAll(_shippingMethodList);
        // Set shipping index to -1 (no selection) for all sellers
        _shippingList[i].shippingIndex = -1;
      } else {
        ApiChecker.checkApi(context, apiResponse);
      }
    }
  ...
  void getAdminShippingMethodListWithoutChosen(BuildContext context) async {
    _isLoading = true;
    _getData = false;
    _shippingList = [];
    // Don't call getChosenShippingMethod to avoid loading previous selections
    ...
    if (apiResponse.response != null &&
      ...
      _shippingList[0].shippingMethodList = [];
      _shippingList[0].shippingMethodList.addAll(_shippingMethodList);
      // Set shipping index to -1 (no selection)
      _shippingList[0].shippingIndex = -1;
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    _isLoading = false;
  ...
  void clearShippingMethodSelection() {
    _chosenShippingList = [];
    _shippingList = [];
    _chosenShippingMethodIndex = [];
    notifyListeners();
  }
  ...
  Future<ResponseModel> checkJdMinimumOrder(BuildContext context) async {
    _isLoading = true;
    notifyListeners();
    ResponseModel responseModel;
    ApiResponse apiResponse = await cartRepo.checkJdMinimumOrder();
    _isLoading = false;
    ...
    if (apiResponse.response != null &&
      ...
      responseModel = ResponseModel(message, success);
    } else {
      String errorMessage = apiResponse.error.toString();
      if (apiResponse.error is String) {
        print(apiResponse.error.toString());
  ...
  Future<double> getJdMinimumOrderAmount() async {
    try {
      ApiResponse apiResponse = await cartRepo.getJdMinimumOrderAmount();

      if (apiResponse.response != null &&
          apiResponse.response.statusCode == 200) {
        Map<String, dynamic> data = apiResponse.response.data;
        return (data['minimum_amount'] ?? 100.0).toDouble();
      } else {
