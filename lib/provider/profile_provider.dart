...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<List<AddressCity>> getAllCity() async {
    final response = await profileRepo.getAllCity();
    print("RESPONSE========= ${response.response.data}");
    if (response.response != null && response.response.statusCode == 200) {
      final list = List<AddressCity>.from(
        response.response.data['products'].map((e) => AddressCity.fromJson(e)),
      );
      return list;
    } else {
      return <AddressCity>[];
