                        ...
                        ListTile(
                          leading: ClipOval(
                            child: Container(
                              ...
                              child: chat.uniqueShopList[index].sellerInfo != null &&
                                  chat.uniqueShopList[index].sellerInfo.image != null &&
                                  chat.uniqueShopList[index].sellerInfo.image.isNotEmpty
                                  ? FadeInImage.assetNetwork(
                                placeholder: Images.placeholder,
                                fit: BoxFit.cover,
                                height: 50,
                                width: 50,
                                image: '${Provider.of<SplashProvider>(context, listen: false).baseUrls.sellerImageUrl}/${chat.uniqueShopList[index].sellerInfo.image}',
                                imageErrorBuilder: (c, o, s) => Image.asset(Images.placeholder, fit: BoxFit.cover, height: 50, width: 50),
                              )
                                  : Image.asset(Images.placeholder, fit: BoxFit.cover, height: 50, width: 50),
                            ),
                          ),
                          title: Text(
                            chat.uniqueShopList[index].sellerInfo != null ?
                            '${chat.uniqueShopList[index].sellerInfo.fName ?? ''} ${chat.uniqueShopList[index].sellerInfo.lName ?? ''}'.trim().isEmpty ?
                            'Unknown Seller' :
                            '${chat.uniqueShopList[index].sellerInfo.fName ?? ''} ${chat.uniqueShopList[index].sellerInfo.lName ?? ''}'.trim() : 'Unknown Seller',
