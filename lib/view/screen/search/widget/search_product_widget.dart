        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(
                  'Search result for \"${Provider.of<SearchProvider>(context).searchText}\" (${products.where((product) => product.status != 0).length} items)',
          ...
          Expanded(
            child: StaggeredGridView.countBuilder(
              ...
              crossAxisCount: 2,
              itemCount: products.where((product) => product.status != 0).length,
              staggeredTileBuilder: (int index) => StaggeredTile.fit(1),
              itemBuilder: (BuildContext context, int index) {
                // Filter products based on status
                final filteredProducts = products.where((product) => product.status != 0).toList();

                // Return ProductWidget with the filtered products
